...
fn min_cost_climbing_stairs_top_bottom(cost: Vec<i32>) -> i32 {
  fn min_cost(n: i32, cost: &Vec<i32>) -> i32 {
    if n < 0 { return 0 } 
    else if n == 0 || n == 1 { return  cost[n as usize] }
...
fn knight_directions() -> [[i32; 2]; 8] {
  const DIRECTIONS : [[i32; 2]; 8] = [[-2, -1], [-2, 1], [-1, 2], [1, 2],
       [2, 1], [2, -1], [1, -2], [-1, -2]];
  DIRECTIONS 
}
...
fn knight_probability_memo(n: i32, k: i32, row: i32, col: i32) -> f64 {
  fn  __knight_probability(n: i32, k: i32, row: i32, col: i32, dp : &mut Vec<Vec<Vec<f64>>>) -> f64 {
    if row < 0 || col < 0 || row >= n || col >= n { return 0.0 }
    if k == 0 { return 1.0 }
    if dp[k as usize][row as usize][col as usize] != -1.0 { return dp[k as usize][row as usize][col as usize] }
    ...
    for row in 0..n {
      for col in 0..n {
        for d in knight_directions() {
          let (prev_row, prev_col) = (row + d[0], col + d[1]);
          if prev_row >= 0 && prev_row < n && prev_col >= 0 && prev_col < n { 
            dp_next[row as usize][col as usize] += dp_prev[prev_row as usize][prev_col as usize] /8.0;
          }      
        }
      }
    }
...
mod test {
  ...
  fn min_cost_climbing_stairs_top_bottom_memo_test() {
    assert_eq!(15, min_cost_climbing_stairs_top_bottom_memo(vec![10,15,20]))
  } 

  #[test]
  fn min_cost_climbing_stairs_top_bottom_memo_opt_test() {
    assert_eq!(15, min_cost_climbing_stairs_top_bottom_memo_opt(vec![10,15,20]))
  } 
